...
class MockAIConfig:
    ...
    def __init__(self):
        ...
        # Add a mock generate method that returns response synchronously
        # but the response itself can have async output methods if needed
        self.generate = MagicMock(return_value=AsyncMockResponse(MOCK_ANALYSIS_RESPONSE))

    def get_model_config(self, model_name):
        # Return a new instance with the same configuration
        config = MockAIConfig()
        config.model_name = model_name
        # Ensure the new config has the same mock behavior
        config.generate = self.generate
        return config
...
def mock_googleai(monkeypatch, mock_gemini):
    ...
    # Mock flow decorator to handle both @flow and @flow(...)
    ...
    mock_genkit.flow.side_effect = mock_flow
    ...
    with (
        ...
        # Apply patches to the reloaded module explicitly
        module.get_ai_config = mock_get_ai_config
...
class TestResumeIntelligencePipeline:
    ...
    async def test_analyze_resume_comprehensive(self, mock_gemini):
        ...
        # Check the dict representation for Pydantic models (robust across reloads)
        result_dict = result.dict() if hasattr(result, "dict") else result.model_dump()
        expected_dict = expected_response.dict() if hasattr(expected_response, "dict") else expected_response.model_dump()
        ...
        # Check prompt contains key resume parts (sanitization resilient)
        prompt_text = mock_gemini.generate_calls[0][0]
        ...
        # Allow float variation or strict check
        # Config is passed as a dict to the generate method
    ...
    async def test_analyze_career_progression(self, mock_gemini):
        ...
        # Call the function (async)
        result = await resume_intelligence_pipeline.analyze_career_progression(SAMPLE_RESUME)
    ...
    async def test_skills_gap_analysis(self, mock_gemini):
        ...
        # Call the function (async)
        result = await resume_intelligence_pipeline.analyze_skills_gap_for_transition(
    ...
    async def test_generate_resume_intelligence_report(self, mock_gemini):
        ...
        import sys
        resume_intelligence_pipeline = sys.modules["app.genkit_flows.resume_intelligence_pipeline"]
