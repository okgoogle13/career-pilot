...
# DOC-007 1.4.2: Soft Skills Taxonomy
SOFT_SKILLS_DB = {
    "leadership", "communication", "teamwork", "problem solving", 
    "stakeholder management", "adaptability", "critical thinking",
    "emotional intelligence", "conflict resolution", "collaboration",
    "interpersonal skills", "time management", "mentoring"
}
...
def _calculate_keyword_score(
    ...
    """
    Calculates a score based on keyword matching, with DOC-007 Soft Skill rules.
    Soft skills are ignored if semantic_score < 70 (Not 'Evidenced').
    """
    # Normalize skills
    resume_skills_lower = {s.lower() for s in resume_skills}
    
    # Helper to check if a skill is 'valid'
    def is_valid_match(skill: str) -> bool:
        skill_lower = skill.lower()
        if skill_lower not in resume_skills_lower:
            return False
            
        # DOC-007 1.4.2: Check Soft Skill Evidence
        is_soft_skill = any(soft in skill_lower for soft in SOFT_SKILLS_DB)
        if is_soft_skill and semantic_score < 70:
            # Soft skill presence without semantic evidence is ignored
            return False
    ...
    # For missing, we check blindly against resume list (to report they are 'technically' there but invalidated? 
    # Or just report as missing? 
    # Current logic: If they were invalidated above, they fall into 'missing' buckets below 
    # because the 'if skill.lower() not in' check needs to be aligned.
    
    # Actually, we need to report them as 'missing' if they didn't pass valid_match.
    # But strictly, 'missing' means NOT IN RESUME.
    # We should distinguish "Missing" vs "Invalidated (No Evidence)".
    # For quick win, we treat them as missing to penalize score.
    
    missing_required = [
        skill
        for skill in job_reqs.requiredSkills
        if skill not in required_matched # If it wasn't matched above, it's missing/invalid
    ]
    missing_preferred = [
        skill
        for skill in job_reqs.preferredSkills
        if skill not in preferred_matched
    ]
...
async def atsScoring(
    ...
    # Step 4: Perform Linguistic Analysis (Authentic Voice Check) [DOC-007 2.3]
    # "Linguistic De-fuzzing" - deterministically flag AI-generated patterns
    AUTH_VOICE_PENALTY_PER_MATCH = 5.0
    FORBIDDEN_PHRASES = [
        "In summary", "Notably", "Spearheaded", "Delved into", 
        "Paramount importance", "Game-changer", "Tapestry", 
        "Testament to", "In conclusion", "Underscores", "By leveraging"
    ]
    
    linguistic_issues = []
    linguistic_penalty = 0.0
    
    resume_lower = resumeText.lower()
    for phrase in FORBIDDEN_PHRASES:
        if phrase.lower() in resume_lower:
            linguistic_issues.append(phrase)
            linguistic_penalty += AUTH_VOICE_PENALTY_PER_MATCH
            
    # Cap penalty at 25 points
    linguistic_penalty = min(linguistic_penalty, 25.0)
    
    # Apply penalty to Semantic Score (as Authentic Voice is a quality metric)
    original_semantic_score = semantic_analysis.similarityScore
    adjusted_semantic_score = max(0.0, original_semantic_score - linguistic_penalty)
    
    # Update explanation if penalties applied
    if linguistic_penalty > 0:
        penalty_msg = f"Score reduced by {int(linguistic_penalty)} pts due to generic AI phrasing: {', '.join([f'{p}' for p in linguistic_issues[:3]])}."
        semantic_analysis.similarityScore = adjusted_semantic_score
        semantic_analysis.explanation = f"{semantic_analysis.explanation} {penalty_msg}"

    # Step 5: Perform Keyword Matching (local operation - always succeeds)
    keyword_analysis_result = await enhanced_ai_handler.execute_ai_operation(
        lambda: _calculate_keyword_score(
            ...
            job_reqs, 
            profileKeywords, 
            semantic_score=semantic_analysis.similarityScore
        ),
        AIOperationContext(
    ...
    # Step 8.1: Add Linguistic Recommendations [DOC-007 2.3]
    if linguistic_issues:
        recommendations.insert(0, f"🛑 AUTHENTICITY CHECK: Your score was penalized for using generic AI terminology. Remove these phrases: '{', '.join(linguistic_issues[:3])}' to restore points.")
