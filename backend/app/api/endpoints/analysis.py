...
async def create_ats_score_analysis(
    ...
    try:
        ...
        # Step 3: Persist to Firestore (Feature 3.2 Persistence)
        from app.core.db import db
        from datetime import datetime

        if db and getattr(current_user, "uid", None):
            user_id = current_user.uid
            try:
                # 1. Save detailed report
                report_data = response_data.model_dump()
                report_data["createdAt"] = datetime.utcnow()
                report_data["jobDescriptionSnippet"] = request.job_description[:200]
                
                db.collection("users").document(user_id).collection("analysis_reports").add(report_data)

                # 2. Update User Profile with latest score (for Quality Gate)
                user_ref = db.collection("users").document(user_id)
                user_ref.set({
                    "latestAtsScore": int(flow_result.overallScore),
                    "lastAnalysisDate": datetime.utcnow()
                }, merge=True)
                
            except Exception as db_error:
                print(f"Warning: Failed to persist analysis results: {db_error}")
...
async def analyze_document(
    ...
    """
    Detailed analysis of a specific document (e.g. resume) to extract
    insights, keywords, and structural feedback. (Implementation Stub)
    """
    # STUB IMPLEMENTATION
    # TODO: Connect to real document analysis flow
    return {
        ...
        "results": {
            "atsScore": 85,
            "keywords": ["React", "Python", "FastAPI"],
            "suggestions": ["Add metrics to experience"],
            "metrics": {
                "readability": 90,
                "impact": 75
            }
            ...
            if ats_score and created_date:
                ...
                if isinstance(created_date, str):
                    try:
                        ...
                        print(f"Warning: Invalid date format in application data: {e}")
                        continue
                else:
