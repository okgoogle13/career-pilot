...
/**
 * M3 Northcote Curio Theme for Material-UI
 * 
 * Configures all MUI components to respect M3 design tokens
 * defined in design-tokens.css. This enables seamless integration
 * between MUI components and the Northcote Curio aesthetic.
 * 
 * **Key Mappings:**
 * - palette.primary → Wattle Gold (#D4A84B)
 * - palette.secondary → Waratah Crimson (#C45C4B)
 * - palette.tertiary → Flannel Flower (#A8A097)
 * - palette.error → M3 Error Red
 * - palette.warning → M3 Warning Amber
 * - shape.borderRadius → M3 organic asymmetry (Pebble, Stone, Leaf)
 * - typography → Federation Stack (Libre Bodoni, Fraunces, Work Sans)
 * 
...
const themeOptions: ThemeOptions = {
    palette: {
        ...
        // Secondary: Terracotta #E09F7D
        secondary: {
            main: '#E09F7D',
            light: '#F2D2C4', // Calculated lighter
            dark: '#D09376',  // Calculated darker
            contrastText: '#2E150F',
        },

        // Tertiary: Wattle Gold #F0C419
    ...
    components: {
        ...
        // Button: M3 filled/outlined/text variants (Pebble shape - friendly)
        MuiButton: {
            styleOverrides: {
                root: {
                    borderRadius: 'var(--sys-shape-pebble)', // Organic archetype, not pill
                    padding: '12px 24px',
                    fontWeight: 'var(--sys-type-weight-medium)',
                    transition: 'all var(--sys-motion-duration-medium-1) var(--sys-motion-easing-expressive-spring)',
                    '&:hover': {
        ...
        // Chip: M3 Gem shape (highlight semantic info)
        MuiChip: {
            styleOverrides: {
                root: {
                    borderRadius: 'var(--sys-shape-gem)', // Organic archetype for emphasis
                    fontFamily: 'var(--sys-type-body-family)',
                    fontWeight: 'var(--sys-type-weight-medium)',
